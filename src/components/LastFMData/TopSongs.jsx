...
export const TopSongs = ({ apiKey, userName, limit, period }) => {
    ...
    const createBarChart = () => {
        ...
        const createGraph = (topSongs) => {
            ...
            //Removes the old svg
            d3.select('#userSongBarChart').select('svg').remove();

            //Creates sources <svg> element
            const userSongBarChartSVG = d3
                .select('#userSongBarChart')
                .append('svg')
                .attr('width', '100%')
                .attr('height', height);

            //Group is used to enforce given marginLeft
            const g = userSongBarChartSVG
                .append('g')
            ...
            //update the scales
            xscale.domain([0, d3.max(topSongs, (d) => d.playCount)]);
            yscale.domain(
                topSongs.map(
                    (d, i) => `${d.artistName} - ${d.songName} - #${i + 1}`
                )
            ); //Mapping on artist and song name
            ...
            userSongBarChartSVG
                .selectAll('text')
                .attr('x', Math.abs(yscale.bandwidth()) * -1 - 20);
            ...
            rect.attr('class', 'bar')
                ...
                .attr('y', (d, i) =>
                    yscale(`${d.artistName} - ${d.songName} - #${i + 1}`)
                )
            ...
            images
                .attr('xlink:href', (d) => d.image) //gets image url
            ...
            //DATA JOIN
            const labels = userSongBarChartSVG
                .selectAll('.playCountLabel')
                .data(topSongs)
                .join(
                    //ENTER
                    //new DOM elements
            ...
            labels
                ...
                .attr('dy', '.75em')
                .transition()
                .delay((d, i) => i * 50) //Animation where the labels fade in one by one
                .style('opacity', 1)
                .text((d) => d.playCount);
        };
    ...
    const filterData = () => {
        ...
        if (error) {
            ...
            lastFMData?.toptracks?.track.forEach((d) => {
                topSongs.push({
                    songName: d.name,
